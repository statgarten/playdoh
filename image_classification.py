        ...
        with st.expander('Model Train', expanded=False):
            ...
            with hyper_parameter_pick:
                with st.container():
                    ...
                    learning_rate = st.text_input('', value = 0.00001, label_visibility='collapsed') 
            ...
            with train_txt:        
                if training :
                    if len(uploaded_images.keys()) !=0 and len(uploaded_images.keys()) >= st.session_state.num_classes:
                        ...
                        st.warning('Please upload train image!!')
                        training = False
        ...
        with st.expander('이미지 업로드', expanded=True):
            ...
            with img_upload:
                ...
                for i in range(st.session_state.num_classes):
                    ...
                    with img_label:
                        class_name = st.text_input(f"클래스 이름 {i+1}", f'클래스 {i+1}')
                        train_labels.append(class_name)
            ...
            with img_see:
                if len(list(uploaded_images.keys())) > 0:
                    ...
                    with pick:
                        label_name = st.radio("미리보기 할 이미지를 선택해주세요",
                                                list(uploaded_images.keys()),
                                                horizontal = True)
                ...
                # 아직 이미지를 업로드 안했을 때
                    ...
                    st.write('미리보기 할 이미지를 선택해주세요')
                    st.info('학습 이미지를 업로드해주세요')

        # model train
        with st.expander('모델 학습', expanded=False):
            # Hyper parameter tuning
            _, hyper_parameter_pick, _, hyper_parameter_explanation, _ = st.columns([0.2, 2, 0.5, 6.5, 0.2]) 
            ...
            with hyper_parameter_pick:
                with st.container():
                    if st.button("**_학습률 :question:_**", type='primary'):
                        st.session_state.explanation = HP_dict['Learning rate']
                    learning_rate = st.text_input('', value = 0.00001, label_visibility='collapsed') 

                with st.container():
                    if st.button("**_배치 사이즈_** :question:", type='primary'):
                        st.session_state.explanation = HP_dict['Batch size']
                    batch_size = st.text_input('', value = 20, label_visibility='collapsed')
                    
                with st.container():
                    if st.button("**_에포크_** :question:", type='primary'):
                        st.session_state.explanation = HP_dict['Epoch']
                    epoch = st.text_input('', value = 100, label_visibility='collapsed')

                with st.container():
                    if st.button("**_최적화 함수_** :question:", type='primary'):
                        st.session_state.explanation = HP_dict['Optimizer']
                    opti = st.selectbox('', ('Adam', 'SGD', 'AdaGrad'), label_visibility='collapsed')
          
            with hyper_parameter_explanation:
                if 'explanation' not in st.session_state:
                    st.session_state.explanation = '각 하이퍼파라미터의 이름 또는 ? 를 클릭하시면 설명을 보실 수 있습니다.'
            ...
            with train_txt:        
                if training :
                    if len(uploaded_images.keys()) !=0 and len(uploaded_images.keys()) >= st.session_state.num_classes:
                        ...
                        # st.markdown(':red[Please upload train image!!]')
                        st.warning('학습 이미지를 업로드 해주세요!')
                        training = False

        # model test
        with st.expander('모델 테스트', expanded=False):
            ...
            with test_img_upload:
                ...
                with test_img_load:
                    ...
                    # 1) Upload a test image and send the image to (POST)
                    test_image = st.file_uploader("테스트 이미지를 업로드 해주세요", type=['png', 'jpg', 'jpeg','tiff','webp'], accept_multiple_files=False)
